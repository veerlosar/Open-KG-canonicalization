...
class VAE_GMM2(nn.Module):
    ...
    """
    def __init__(self, dims, n_clusters, init_clust_assignment_file, 
    weights, pretrained_models, term2idx, device, umaps):
        super(VAE_GMM2, self).__init__()
        '''Adding umaps for dim reduction'''
        self.umaps = umaps
        self.device = device
    ...
    def __initialize_embeddings(self, pretrained_model, source: str, term2idx: Dict[str, int]):
        ...
        print('''Reducing dimensionality with umap''', f'from {weights_matrix.shape} to {input_dims}')
        wmatrix = self.umaps[int(input_dims)].fit_transform(weights_matrix)
